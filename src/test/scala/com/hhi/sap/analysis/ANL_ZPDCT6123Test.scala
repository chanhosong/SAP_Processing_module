...
class ANL_ZPDCT6123Test extends FlatSpec with SparkSessionTestWrapper{
  ...
  "ZPDCT6123 " should "make dataframe." in new SparkFileReader {
    ...
    new ANL_ZPDCT6123(ss.sqlContext).run(ZPDCT6123, tb_ZPSCT600, eban.select(TERM_MASTER.EBAN.BANFN, TERM_MASTER.EBAN.BFNPO, TERM_MASTER.EBAN.LGORT, TERM_MASTER.EBAN.PAINTGBN), mara.select(TERM_MASTER.MARA.MATNR, TERM_MASTER.MARA.ZZMGROUP)).show(100)
  }
}